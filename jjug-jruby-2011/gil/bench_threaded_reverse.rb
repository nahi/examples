...
class ThreadScalability
  ...
  def do_test()
    ...
    concurrency.times { |i|
      threads << Thread.new {
        puts "started thread #{i}" if $DEBUG
  ...
  def reverse_strings(beginIndex, endIndex)
    while beginIndex <= endIndex
      puts "another 10 in #{Thread.current.inspect}" if beginIndex % 10 == 0 and $DEBUG
...
5.times {
  ThreadScalability.new.run((ARGV.shift || "10").to_i)
}
